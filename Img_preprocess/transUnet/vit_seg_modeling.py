"""
This part of the code is from TransUNet project
https://github.com/Beckschen/TransUNet
The code was modified slightly to be able to take the four-channel images as the input
"""
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if self.transformer.embeddings.hybrid:
                self.transformer.embeddings.hybrid_model.root1.conv.weight.copy_(np2th(res_weight["conv_root/kernel"], conv=True))
                gn_weight = np2th(res_weight["gn_root/scale"]).view(-1)
                gn_bias = np2th(res_weight["gn_root/bias"]).view(-1)
                self.transformer.embeddings.hybrid_model.root1.gn.weight.copy_(gn_weight)
                self.transformer.embeddings.hybrid_model.root1.gn.bias.copy_(gn_bias)

                self.transformer.embeddings.hybrid_model.root2.conv.weight.copy_(np2th(res_weight["conv_root/kernel"], conv=True))
                self.transformer.embeddings.hybrid_model.root2.gn.weight.copy_(gn_weight)
                self.transformer.embeddings.hybrid_model.root2.gn.bias.copy_(gn_bias)
